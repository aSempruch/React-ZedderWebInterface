...
def whoCovered(shiftId, site, zed_date):
	r = s.get('https://zed.rutgers.edu/scheduling/view/'
		+ str(work_sites_index[site])
		+ '/?start_date='
		+ zed_date,
		cookies=cookies)
	soup = BeautifulSoup(r.text, 'lxml')
	owner = soup.find('div', class_='schedule').find('table').find('tbody').find('tr').find('div', {"id": "scheduleShiftPosted_"+shiftId}).find('p', class_='owner')
	owner = owner.getText().strip()
	return owner
	...
	for cell in s_table:
		...
		for shifts in shiftsCol:
			if shifts is not None and shifts.find('input') is not None:
				...
				lock.acquire()
				new_scanned[shift_id] = [str(shifts.find('div').contents[0]).strip().split()[2], work_sites_name[siteIndex]]
				lock.release()
	return 0
...
cnx = mysql.connector.connect(user='zedder', password='zeddershredder', host='127.0.0.1',database='zedder')
cursor = cnx.cursor()

sql_newpost = ("INSERT INTO Data "
               "(shiftId, site, posted, covered, postedTime, coveredTime) "
               "VALUES (%s, %s, %s, NULL, %s, NULL)")

#Check if something was posted
for shiftId, data in new_scanned.iteritems():
    if str(shiftId) not in dict_scanned:
        sendMail("DB Add", "Added new shift " + shiftId + " to database",'')
        data_newpost = (shiftId, data[1], data[0], datetime.datetime.now())
        cursor.execute(sql_newpost, data_newpost)

sql_newcover = ("UPDATE Data "
				"SET covered=%s, coveredTime=%s"
				"WHERE shiftId=%s AND posted=%s")

#Check if something was covered
for shiftId, data in dict_scanned.iteritems():
	if str(shiftId) not in new_scanned:
		sendMail("DB Update ", shiftId + " posted by " + data[0] +" was covered",'');
		data_newcover = (whoCovered(shiftId, data[1], str(date)[:10]), datetime.datetime.now(), shiftId, data[0])
		cursor.execute(sql_newcover, data_newcover)
cnx.commit()
cursor.close()
cnx.close()
